...
if __name__ == '__main__':
    ...
    # horizontal items
    for row in grid:
        for c in xrange(0, len(row) - 4):
            p = prod(row[c:c+4])
            if p > max_prod:
                max_prod = p
    # vertical items
    for r in xrange(0, len(grid) - 3):
        for c, rc in enumerate(grid[r]):
            p = prod([rc, grid[r+1][c], grid[r+2][c], grid[r+3][c]])
            if p > max_prod:
                max_prod = p
    # down and right diagonals
    for r in xrange(0, len(grid) - 3):
        for c in xrange(0, len(grid[r]) - 3):
            p = prod([grid[r+x][c+x] for x in xrange(4)])
            if p > max_prod:
                max_prod = p
    # down and left diagonals
    for r in xrange(0, len(grid) - 3):
        for c in xrange(3, len(grid[r])):
            p = prod([grid[r+x][c-x] for x in xrange(4)])
            if p > max_prod:
                max_prod = p
